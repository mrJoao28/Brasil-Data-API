...
import { managedCnpjProvider } from './managedCnpjProvider';
import type { CnpjProvider } from './cnpjProvider';

export function getCnpjProvider(): CnpjProvider {
  switch (env.CNPJ_PROVIDER) {
    case 'brasilapi': return brasilApiCnpjProvider;
    case 'opencnpj': return openCnpjProvider;
    case 'failover': return managedCnpjProvider;
    case 'receita-federal':
      throw new Error('CNPJ_PROVIDER=receita-federal is not implemented yet. It requires OAuth2 client credentials registered at gov.br/conecta. See docs/PROVIDERS.md.');
    default: {
